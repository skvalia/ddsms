...
function NewDssrForm() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: p }, { data: dt }, { data: des }, { data: mt }, { data: lastDssr }, { data: pendingCounts }] = await Promise.all([
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (machineTypeId) {
      ...
      if (mt) insertData.machine_type = mt.name;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 pb-16">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight">New Design Folder (DSSR)</h1>
        <p className="text-sm text-stone-500 mt-1">
          System No: <span className="font-semibold text-amber-700">{nextNo}</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white border border-stone-200 rounded-2xl p-4 space-y-4">
          ...
          <Field label="Your Reference No"
            hint="Your existing DSSR/folder number from your old system">
            <input value={yourRefNo} onChange={(e) => setYourRefNo(e.target.value)}
              placeholder="e.g. F-2024-089" className={INPUT} />
          </Field>
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-4 space-y-4">
          ...
          <SearchableSelect label="Machine Type" table="machine_types"
            value={machineTypeId} onChange={setMachineTypeId}
            options={machineTypes} setOptions={setMachineTypes}
            placeholder="Schiffli, Multi, Aari..." />
          ...
          <SearchableSelect label="Assigned Designer" table="designers"
            ...
            hint="Badge shows current pending work" />

          <Field label="Description">
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              rows={2} placeholder="Brief description of this design folder..."
