...
export default async function SketchDetailPage({
  ...
  const { data: sketch } = await supabase.from("sketches").select("*, inspiration:inspirations(id, concept_name)").eq("id", id).maybeSingle();
  if (!sketch) return notFound();

  const { data: dssrs } = await supabase.from("dssr").select("*, party:parties(name)").eq("sketch_id", id).order("created_at", { ascending: false });

  const photoUrl = sketch.photo_path ? `${SUPABASE_URL}/storage/v1/object/public/sketch-files/${sketch.photo_path}` : null;
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2 text-sm text-stone-400">
            ...
            {sketch.inspiration && (
              <><span>→</span>
              <Link href={`/inspirations/${sketch.inspiration.id}`} className="hover:text-stone-600">{sketch.inspiration.concept_name}</Link></>
            )}
            <span>→</span>
            <span className="text-stone-700 font-medium">{sketch.sketch_number}</span>
          </div>
          <Link href={`/sketches/${id}/edit`} className="text-sm font-semibold text-amber-700 border border-amber-200 px-3 py-1.5 rounded-lg">
            ✏️ Edit
          </Link>
        </div>
        ...
        <div className="mb-5">
          ...
          {sketch.sketched_by && <p className="text-sm text-stone-500 mt-1">✏️ Sketched by {sketch.sketched_by}</p>}
          {sketch.design_number && <p className="text-sm font-semibold text-amber-700 mt-1">Design No: {sketch.design_number}</p>}
          {sketch.description && <p className="mt-3 text-sm text-stone-600 bg-stone-50 rounded-xl px-4 py-3">{sketch.description}</p>}
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-bold uppercase tracking-wider text-stone-400">Design Records / DSSR ({dssrs?.length ?? 0})</h2>
            <Link href={`/dssr/new?sketch=${id}${sketch.inspiration ? `&inspiration=${sketch.inspiration.id}` : ""}`}
