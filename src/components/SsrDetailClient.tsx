...
import {
  ...
  type Ssr, type SsrFile, type SsrFileStage,
  type StatusHistory, type Comment, type SsrStatus, type IssueType,
} from "@/types/database";
...
export function SsrDetailClient({
  ...
  async function handleStatusChange(newStatus: SsrStatus) {
    ...
    if (newStatus === "Issue" && issueType) updates.issue_type = issueType;
    if (newStatus !== "Issue") updates.issue_type = null;
    const { error } = await supabase.from("ssr").update(updates).eq("id", ssr.id);
    if (!error) {
      setCurrentStatus(newStatus);
      setUploadStage(defaultStageForStatus(newStatus));
      setLocalHistory((prev) => [{
        id: crypto.randomUUID(), ssr_id: ssr.id,
        old_status: currentStatus, new_status: newStatus,
        changed_by: userId, remarks: remarks || null,
        changed_at: new Date().toISOString(),
      }, ...prev]);
      setRemarks("");
      await supabase.from("activity_logs").insert({
        entity_type: "ssr", entity_id: ssr.id,
  ...
  async function handleUpload(file: File) {
    ...
    if (!error) {
      ...
      const { data, error: insertError } = await supabase.from("ssr_files").insert({
        ssr_id: ssr.id, file_path: path, file_type: ext,
        stage: uploadStage, status_history_id: latestHistoryId, uploaded_by: userId,
      }).select().single();
      if (!insertError && data) {
        setLocalFiles((prev) => [data as SsrFile, ...prev]);
        await supabase.from("activity_logs").insert({
          entity_type: "ssr", entity_id: ssr.id, action: "file_uploaded",
          description: `${uploadStage} file uploaded: ${file.name}`, created_by: userId,
        });
      }
    }
    setUploading(false);
  ...
  async function handleAddComment() {
    ...
    if (!error && data) {
      ...
      setCommentText("");
      await supabase.from("activity_logs").insert({
        entity_type: "ssr", entity_id: ssr.id, action: "comment_added",
        description: commentText.trim().slice(0, 100), created_by: userId,
      });
    }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-8">
      ...
      <div className="flex items-start justify-between gap-3 mb-1">
        <div>
          ...
          <p className="text-sm text-(--color-ink-soft) font-(family-name:--font-mono) mt-0.5">
            SSR {ssr.ssr_number}{ssr.sample_no ? ` · ${ssr.sample_no}` : ""}
          </p>
        </div>
        <div className="flex flex-col items-end gap-1.5 mt-1">
          <StatusPill label={currentStatus} color={ssrStatusColor[currentStatus]} bg={ssrStatusBg[currentStatus]} />
          <Link href={`/ssr/${ssr.id}/edit`} className="flex items-center gap-1 text-xs text-(--color-thread) font-medium">
      ...
      {/* Costing tab */}
      {activeTab === "costing" && (
        <CostingPanel ssrId={ssr.id} initial={ssr as any} />
      )}
      ...
      {activeTab === "details" && (
        <>
          ...
          <div className="bg-(--color-surface) border border-(--color-line) rounded-2xl p-4 mb-5">
            ...
            <dl className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
              ...
              {ssr.party_dispatch_date && (
                <DetailItem label="Dispatched" value={format(new Date(ssr.party_dispatch_date), "dd MMM yyyy")} />
              )}
            </dl>
            {ssr.remarks && (
              <div className="mt-3 pt-3 border-t border-(--color-line)">
                <p className="text-xs font-semibold uppercase tracking-wider text-(--color-ink-soft) mb-1">Remarks</p>
          ...
          <div className="bg-(--color-surface) border border-(--color-line) rounded-2xl p-4 mb-5">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {SSR_STATUSES.map((status) => {
                ...
                return (
                  <button key={status} disabled={statusLoading} onClick={() => handleStatusChange(status)}
            ...
            {currentStatus === "Issue" && (
              <div className="mb-3">
                <label className="block text-xs font-medium mb-1.5 text-(--color-ink-soft)">Issue Type</label>
            ...
            <h3 className="text-xs font-semibold uppercase tracking-wider text-(--color-ink-soft) mt-5 mb-3">Timeline</h3>
          ...
          <div className="bg-(--color-surface) border border-(--color-line) rounded-2xl p-4 mb-5">
            ...
            <div className="mb-3">
              <label className="block text-xs font-medium mb-1.5 text-(--color-ink-soft)">Tag new upload as</label>
              <select value={uploadStage} onChange={(e) => setUploadStage(e.target.value as SsrFileStage)}
                className="w-full rounded-lg border border-(--color-line) bg-(--color-paper) px-3 py-2 text-sm">
                {SSR_FILE_STAGES.map((stage) => <option key={stage} value={stage}>{stage}</option>)}
              </select>
            </div>
            <FileGallery files={localFiles} bucket="ssr-files"
              getPublicUrl={(path) => bucketPublicUrl("ssr-files", path)}
              onUpload={handleUpload} uploading={uploading} groupByStage />
          </div>

          {/* Comments */}
          <div className="bg-(--color-surface) border border-(--color-line) rounded-2xl p-4 mb-5">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-(--color-ink-soft) mb-3">Comments</h2>
            ...
            <div className="space-y-3">
              {localComments.map((c) => (
                <div key={c.id} className="text-sm">
                  <p>{c.body}</p>
                  <p className="text-xs text-(--color-ink-soft) mt-0.5">{format(new Date(c.created_at), "dd MMM yyyy, HH:mm")}</p>
                </div>
              ))}
              {localComments.length === 0 && <p className="text-sm text-(--color-ink-soft)">No comments yet.</p>}
            </div>
          </div>
        </>
      )}
...
function defaultStageForStatus(status: SsrStatus): SsrFileStage {
  switch (status) {
    case "YTR": return "Concept";
    case "On Machine": return "On Machine";
    case "Mending": return "On Machine";
    case "Dyeing": return "Dyeing";
    case "Packing":
    case "Done":
    case "Completed": return "Sample Photo";
    case "Sent To Outside Emb": return "EMB";
